  ...
  return (
    <div className='bg-black text-white flex justify-around items-center flex-col sm:flex-row'>
      ...
      <div className='flex gap-2 items-center'>
        {!islogin && (
          <button className='bg-red-500 px-4 py-1 rounded-md text-xl cursor-pointer' onClick={() => navigate('/signup')}>
            Sign up
          </button>
        )}
        {islogin ? (
          <button className='bg-blue-500 px-4 py-1 rounded-md text-xl cursor-pointer' onClick={logoutHandler}>
            Logout
          </button>
        ) : (
          <button className='bg-blue-500 px-4 py-1 rounded-md text-xl cursor-pointer' onClick={() => navigate('/login')}>
            Login
          </button>
        )}
        {islogin && (
          <button className='bg-red-500 px-4 py-1 rounded-md text-xl cursor-pointer' onClick={() => navigate('/user')}>
            Progress
          </button>
        )}
        {islogin && (
